...
func ReadGenesis(db ethdb.Database) (*Genesis, error) {
	...
	genesis.Timestamp = genesisHeader.SecondsTimestamp()
...
func SetupGenesisBlockWithOverride(db ethdb.Database, triedb *triedb.Database, genesis *Genesis, overrides *ChainOverrides) (*params.ChainConfig, common.Hash, error) {
	...
	compatErr := storedcfg.CheckCompatible(newcfg, head.Number.Uint64(), head.SecondsTimestamp())
	if compatErr != nil && ((head.Number.Uint64() != 0 && compatErr.RewindToBlock != 0) || (head.SecondsTimestamp() != 0 && compatErr.RewindToTime != 0)) {
