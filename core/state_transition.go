...
func (st *StateTransition) buyGas() error {
	...
	if st.evm.Context.L1CostFunc != nil && !st.msg.SkipAccountChecks {
		if st.msg.GasPrice.Cmp(big.NewInt(0)) == 0 && st.evm.ChainConfig().IsWright(st.evm.Context.CurrentTime()) {
			l1Cost = big.NewInt(0)
		} else {
			// ***************** //
			// should double check the L1CostFunc params block time //
			// ***************** //
			l1Cost = st.evm.Context.L1CostFunc(st.msg.RollupCostData, st.evm.Context.CurrentTime())
		}
		if l1Cost != nil {
	...
	if st.evm.ChainConfig().IsCancun(st.evm.Context.BlockNumber, st.evm.Context.CurrentTime()) {
...
func (st *StateTransition) preCheck() error {
	if st.msg.IsDepositTx {
		...
		if st.msg.IsSystemTx {
			if st.evm.ChainConfig().IsOptimismRegolith(st.evm.Context.CurrentTime()) {
...
func (st *StateTransition) TransitionDb() (*ExecutionResult, error) {
	...
	if err != nil && err != ErrGasLimitReached && st.msg.IsDepositTx {
		...
		if st.msg.IsSystemTx && !st.evm.ChainConfig().IsRegolith(st.evm.Context.CurrentTime()) {
			gasUsed = 0
		}
...
func (st *StateTransition) innerTransitionDb() (*ExecutionResult, error) {
	...
	var (
		...
		rules            = st.evm.ChainConfig().Rules(st.evm.Context.BlockNumber, st.evm.Context.Random != nil, st.evm.Context.CurrentTime())
	...
	if optimismConfig := st.evm.ChainConfig().Optimism; optimismConfig != nil && rules.IsOptimismBedrock && !st.msg.IsDepositTx {
		...
		} else if l1Cost := st.evm.Context.L1CostFunc(st.msg.RollupCostData, st.evm.Context.CurrentTime()); l1Cost != nil {
			// *********** //
