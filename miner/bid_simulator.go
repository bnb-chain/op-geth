...
type bidWorker interface {
	...
	fillTransactions(interrupt *atomic.Int32, env *environment, bidTxs mapset.Set[common.Hash]) (err error)
}
...
func (b *bidSimulator) newBidLoop() {
	...
	for {
		...
		case newBid := <-b.newBidCh:
			...
			expectedValidatorReward := newBid.GasFee
...
func (b *bidSimulator) bidMustBefore(parentHash common.Hash) time.Time {
	...
	return bidutil.BidMustBefore(parentHeader, 1, b.delayLeftOver)
}

func (b *bidSimulator) bidBetterBefore(parentHash common.Hash) time.Time {
	parentHeader := b.chain.GetHeaderByHash(parentHash)
	// TODO using 1s as block period temp
	return bidutil.BidBetterBefore(parentHeader, 1, b.delayLeftOver, b.config.BidSimulationLeftOver)
}
...
func (b *bidSimulator) simBid(interruptCh chan int32, bidRuntime *BidRuntime) {
	...
	// TODO update compare gas price logic
	...
	//		}
	//
	//		fillErr := b.bidWorker.fillTransactions(interruptCh, bidRuntime.env, bidTxsSet)
...
// TODO this file no builder part, just sequencer and op no blob tx
