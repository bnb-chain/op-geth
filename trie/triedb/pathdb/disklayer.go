...
type trienodebuffer interface {
	...
	// copy.
	commit(block uint64, nodes map[common.Hash]map[string]*trienode.Node) trienodebuffer
...
func (dl *diskLayer) commit(bottom *diffLayer, force bool) (*diskLayer, error) {
	...
	ndl := newDiskLayer(bottom.root, bottom.stateID(), dl.db, dl.cleans, dl.buffer.commit(bottom.block, bottom.nodes))
